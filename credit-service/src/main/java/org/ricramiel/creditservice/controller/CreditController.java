...
public class CreditController {
    ...
    public void deleteCredit(@PathVariable UUID creditId){
        creditService.deleteCredit(creditId);
    }

    @GetMapping("/{userId}/get_by_user_id")
    public ResponseEntity<List<Credit>> getByUserId(@PathVariable UUID userId){
        return ResponseEntity.ok(creditService.getByUserId(userId));
    }

    @GetMapping("/{cardAccountId}/get_by_card_account")
    public ResponseEntity<Credit> getByCardAccountId(@PathVariable UUID cardAccountId){
        return ResponseEntity.ok(creditService.getByCardAccountId(cardAccountId));
    }

    @PostMapping("/{cardAccountId}/enrollment")
    public ResponseEntity<Credit> makeEnrollment(@PathVariable UUID cardAccountId,@RequestParam BigDecimal money){
