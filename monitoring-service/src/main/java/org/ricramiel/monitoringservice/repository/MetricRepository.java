...
public interface MetricRepository extends JpaRepository<MetricEntity, UUID> {
    List<MetricEntity> findByTimeAfterOrderByTimeDesc(LocalDateTime time);

    List<MetricEntity> findTop100ByOrderByTimeDesc();

    List<MetricEntity> findByTraceIdOrderByTimeAsc(String traceId);
}
